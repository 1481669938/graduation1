...
with tf.Session() as sess:
    ...
    for p in range(1500000):
        for j in range(100):
            i1 = np.random.randint(0, 5, [1])  # 从哪个文件夹里抽
            i2 = np.random.randint(pca_data_train.shape[0]*i1/5, pca_data_train.shape[0]*(i1+1)/5, [10])  # 从哪个文件夹里抽
            data_image_train_pca.append(np.mean(pca_data_train[i2],axis=0))
            data_label_train_pca.append(np.mean(train_data_lable[i2],axis=0))

            i2 = np.random.randint(pca_data_train.shape[0] * i1 / 5, pca_data_train.shape[0] * (i1 + 1) / 5, [10])
            data_image_train_rbf.append(np.mean(train_data[i2], axis=0))
            data_label_train_rbf.append(np.mean(train_data_lable[i2], axis=0))

            i2 = np.random.randint(pca_data_train.shape[0]*i1/5, pca_data_train.shape[0]*(i1+1)/5, [10])
            data_image_train_bp.append(np.mean(train_data[i2], axis=0))
            data_label_train_bp.append(np.mean(train_data_lable[i2], axis=0))
            # print(data_label_train_pca)
            # print(data_label_train_rbf)
            # print(data_label)
        ...
        if p % 1000 == 0:
            for j in range(100):
                i1 = np.random.randint(0, 5, [1])  # 从哪个文件夹里抽
                i2 = np.random.randint(pca_data_test.shape[0] * i1 / 5, pca_data_test.shape[0] * (i1 + 1) / 5,
                                       [10])  # 从哪个文件夹里抽
                data_image_test_pca.append(np.mean(pca_data_test[i2], axis=0))
                # data_label_train_pca.append(np.mean(train_data_lable[i2], axis=0))

                data_image_test_rbf.append(np.mean(test_data[i2], axis=0))
                # data_label_train_rbf.append(np.mean(train_data_lable[i2], axis=0))

                data_image_test_bp.append(np.mean(test_data[i2], axis=0))
                data_label_test.append(np.mean(test_data_lable[i2], axis=0))
        ...
        if p % 1000 == 0:
            for j in range(100):
                i1 = np.random.randint(0, 5, [1])  # 从哪个文件夹里抽
                i2 = np.random.randint(pca_data_vary.shape[0] * i1 / 5, pca_data_vary.shape[0] * (i1 + 1) / 5,
                                       [10])  # 从哪个文件夹里抽
                data_image_vary_pca.append(np.mean(pca_data_vary[i2], axis=0))
                # data_label_train_pca.append(np.mean(train_data_lable[i2], axis=0))

                data_image_vary_rbf.append(np.mean(vary_data[i2], axis=0))
                # data_label_train_rbf.append(np.mean(train_data_lable[i2], axis=0))

                data_image_vary_bp.append(np.mean(vary_data[i2], axis=0))
                data_label_vary.append(np.mean(vary_data_lable[i2], axis=0))
