...
train_chara_1_label = np.loadtxt(r"E:\pycharm\tensorflow\data\sort2\train_chara_10_20_whole_label.txt")
train_chara_2_label = np.loadtxt(r"E:\pycharm\tensorflow\data\sort2\train_chara_20_30_whole_label.txt")
train_chara_3_label = np.loadtxt(r"E:\pycharm\tensorflow\data\sort2\train_chara_30_40_whole_label.txt")
train_chara_4_label = np.loadtxt(r"E:\pycharm\tensorflow\data\sort2\train_chara_40_50_whole_label.txt")
train_chara_5_label = np.loadtxt(r"E:\pycharm\tensorflow\data\sort2\train_chara_50_60_whole_label.txt")
test_chara_1_label = np.loadtxt(r"E:\pycharm\tensorflow\data\sort2\test_chara_10_20_whole_label.txt")
test_chara_2_label = np.loadtxt(r"E:\pycharm\tensorflow\data\sort2\test_chara_20_30_whole_label.txt")
test_chara_3_label = np.loadtxt(r"E:\pycharm\tensorflow\data\sort2\test_chara_30_40_whole_label.txt")
test_chara_4_label = np.loadtxt(r"E:\pycharm\tensorflow\data\sort2\test_chara_40_50_whole_label.txt")
test_chara_5_label = np.loadtxt(r"E:\pycharm\tensorflow\data\sort2\test_chara_50_60_whole_label.txt")
# test_chara_1_label = np.loadtxt(r"E:\pycharm\tensorflow\data\sort2\test_chara_10_20_whole_label.txt")
vary_chara_2_label = np.loadtxt(r"E:\pycharm\tensorflow\data\check\20_30_label.txt")
vary_chara_3_label = np.loadtxt(r"E:\pycharm\tensorflow\data\check\30_40_label.txt")
vary_chara_4_label = np.loadtxt(r"E:\pycharm\tensorflow\data\check\40_50_label.txt")
# test_chara_5_label = np.loadtxt(r"E:\pycharm\tensorflow\data\sort2\test_chara_50_60_whole_label.txt")
...
train_data_label = np.concatenate((train_chara_1_label,train_chara_2_label,train_chara_3_label,train_chara_4_label,train_chara_5_label), axis=0)
test_data_label = np.concatenate((test_chara_1_label,test_chara_2_label,test_chara_3_label,test_chara_4_label,test_chara_5_label), axis=0)
vary_data_label = np.concatenate((vary_chara_2_label,vary_chara_3_label,vary_chara_4_label), axis=0)# data_image.append(train_chara_2_spit[0][0])
...
def hidden_layer_pca(input_tensor,keep_prob, regularizer,avg_class,resuse):
    ...
    Full_connection4_weights = tf.Variable(tf.random_normal([100, 1]))
    #if regularizer != None:
    tf.add_to_collection("losses_pca", regularizer(Full_connection4_weights))
    Full_connection4_biases = tf.Variable(tf.random_normal([1]))
    # variables_averages_op = avg_class.apply(tf.trainable_variables())
    if avg_class == None:
...
y_pca = tf.placeholder(tf.float32, [None, 1], name="y-input_pca")

x_rbf = tf.placeholder(tf.float32, [batch_size ,trait],name="x-input_rbf")
y_rbf = tf.placeholder(tf.float32, [None, 1], name="y-input_rbf")
...
cross_entropy_mean_pca = tf.reduce_sum(tf.pow(y1_pca - y_pca, 2))/100
...
cross_entropy_mean_rbf = tf.reduce_sum(tf.pow(y1_rbf - y_rbf, 2))/100
# cross_entropy = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=y1, labels=tf.argmax(y_, 1))
# cross_entropy_mean = tf.reduce_mean(cross_entropy)
cross_entropy_mean = tf.reduce_sum(tf.pow(y1 - y_, 2))/100
...
y_result = 0.25*y1_pca+0.5*y1_rbf+0.25*y1
...
accuracy = tf.reduce_sum(tf.abs(y_ - y_result)/y_)/100
...
with tf.Session() as sess:
    # saver.restore(sess, r"E:\pycharm\tensorflow\data\sort2\2.bag/1.ckpt")
    ...
    for p in range(150000):
        ...
        if p % 1000 == 0:
            ...
            for i in i1:
                ...
                data_label_test.append(test_data_label[i])
            ...
            data_label = np.vstack(data_label_test).reshape(1, 100, 1)
            ...
            if p % 1000 == 0:
                ...
                for i in i1:
                    ...
                    data_label_vary.append(vary_data_label[i])
                ...
                data_label = np.vstack(data_label_vary).reshape(1, 100, 1)
                mse = sess.run(accuracy, feed_dict={x_pca: data_image_pca[0][:, 0:trait_pca], x_rbf: data_image_rbf[0],
                                                    x_: data_image[0], y_: data_label[0], keep_prob: 0.5})
                print(p, mse)
            # y_numpy.append(
            #     sess.run(y_result, feed_dict={x_pca: data_image_pca[0][:,0:trait_pca], x_rbf: data_image_rbf[0],
            #                                   x_: data_image[0], keep_prob: 0.5}))
            # y_label.append(data_label_test)
            # y_numpy = np.vstack(y_numpy).reshape(100, 1)
            # y_label = np.vstack(y_label).reshape(100, 1)
            # np.savetxt(r"E:\pycharm\tensorflow\data\sort2\numpy_bag.txt", y_numpy)
            # np.savetxt(r"E:\pycharm\tensorflow\data\sort2\label_bag.txt", y_label)
            # j = []
            # for k in range(50):
            #     i1 = np.random.randint(0, 100, [1])  # 从哪个文件夹里抽
            #     j.append(i1)
            # j = np.vstack(j).reshape(50, 1)
            # y_numpy_1 = np.loadtxt(r"E:\pycharm\tensorflow\data\sort2\numpy_bag.txt").reshape(100, 1)
            # y_label_1 = np.loadtxt(r"E:\pycharm\tensorflow\data\sort2\label_bag.txt").reshape(100, 1)
            # i = np.arange(0, 50).reshape(50, 1)
            # # print(j)
            # plt.figure(1)
            # plt.title("Bagging网络", size=15)
            # plt.xlabel('泡沫图像序列', size=15)
            # plt.ylabel('泡沫图像品位', size=15)
            #
            # plt.plot()
            # plt.plot(i, y_numpy_1[j].reshape(50, 1), c="red")
            # plt.plot(i, y_label_1[j].reshape(50, 1), c="blue")
            # plt.legend(["预测值", "真实值"])
            # data_image_pca = []
            # data_image_rbf = []
            # data_image = []
    ...
    # for p in range(150000):
    #     for k in range(100):
    #         i1 = np.random.randint(0, test_data.shape[0], [1])  # 从哪个文件夹里抽
    #         data_image_test.append(test_data[i1])
    #         data_label_test.append(test_data_label[i1])
    #     data_image_test = np.vstack(data_image_test).reshape(1, 100, 40)
    #     data_label_test = np.vstack(data_label_test).reshape(1, 100, 1)
    #     data_image_test_pca = pca.transform(data_image_test[0])
    #     acc = sess.run(100*accuracy, feed_dict={x_pca: data_image_test_pca[:, 0:20], x_rbf: data_image_test[0],
    #                                         x_: data_image_test[0], y_: data_label_test[0], keep_prob: 0.5})
    #     # acc = 100 * sess.run(accuracy, feed_dict={x: data_image[0], y_: data_label[0], keep_prob: 0.5})
    #     # print("In %d circle, the accuracy of 100 picture is %g%%" % (p, acc))
    #     # ave.append(acc)
    #     y_numpy.append(sess.run(y_result, feed_dict={x_pca: data_image_test_pca[:, 0:20], x_rbf: data_image_test[0],
    #                                           x_: data_image_test[0], keep_prob: 0.5}))
    #     y_label.append(data_label_test)
    #     print("In %d circle, the  average margin of error  of 100 picture is %g%%" % (p, acc))
    #     ave.append(acc)
    #     data_image_test = []
    #     data_label_test = []
    # print("Bagging网络平均误差评估为%g%%"%((sum(ave) - max(ave) - min(ave)) / (len(ave) - 2)))
    # y_numpy = np.vstack(y_numpy).reshape(1000, 1)
    # y_label = np.vstack(y_label).reshape(1000, 1)
    # np.savetxt(r"E:\pycharm\tensorflow\data\sort2\numpy_bag.txt", y_numpy)
    # np.savetxt(r"E:\pycharm\tensorflow\data\sort2\label_bag.txt", y_label)
    # j = []
    # for k in range(50):
    #     i1 = np.random.randint(0, 1000, [1])  # 从哪个文件夹里抽
    #     j.append(i1)
    # j = np.vstack(j).reshape(50, 1)
    # y_numpy_1 = np.loadtxt(r"E:\pycharm\tensorflow\data\sort2\numpy_bag.txt").reshape(1000, 1)
    # y_label_1 = np.loadtxt(r"E:\pycharm\tensorflow\data\sort2\label_bag.txt").reshape(1000, 1)
    # i = np.arange(0, 50).reshape(50, 1)
    # # print(j)
    # plt.figure(1)
    # plt.title("Bagging网络", size=15)
    ...
    # plt.plot(i, y_label_1[j].reshape(50, 1), c="blue")
    # plt.legend(["预测值", "真实值"])
    # # print(y_numpy_1[0:50])
    # # print(y_label_1[0:50])
    # plt.show()
    # saver.save(sess, r"E:\pycharm\tensorflow\data\sort2\2.bag/1.ckpt")
